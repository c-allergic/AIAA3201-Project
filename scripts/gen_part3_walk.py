#!/usr/bin/env python3
"""Generate Part 3 comparison figure on Vid4 'walk' sequence."""
import os, sys
import numpy as np
...
FRAME_COUNT = 6
FRAME_START = 8
FRAME_STEP = 6  # walk has more frames (~50+), spread wider
...
for label, dirname in METHODS:
    ...
    if not os.path.isdir(d):
        print(f"MISSING: {d}")
        # Try alternate names
        for alt in [f"bi_{SEQUENCE}_basicvsr_pp", f"part3_{SEQUENCE}_C_hybrid_g0.3"]:
            alt_d = os.path.join(BASE, alt)
            if os.path.isdir(alt_d):
    ...
    print(f"[{label}] {len(files)} frames in {os.path.basename(d)}")
